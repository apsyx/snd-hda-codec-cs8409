...
void cs8409_cs42l42_fixups(struct hda_codec *codec, const struct hda_fixup *fix, int action)
{
	...
	case HDA_FIXUP_ACT_PRE_PROBE:
		...
		spec->variant_ops = &cs8409_cs42l42_patch_ops;
...
void dolphin_fixups(struct hda_codec *codec, const struct hda_fixup *fix, int action)
{
	...
	case HDA_FIXUP_ACT_PRE_PROBE:
		...
		spec->variant_ops = &cs8409_dolphin_patch_ops;
...
static int cs8409_dispatch_build_controls(struct hda_codec *codec)
{
	struct cs8409_spec *spec = codec->spec;
	if (spec && spec->variant_ops && spec->variant_ops->build_controls)
		return spec->variant_ops->build_controls(codec);
	return 0;
}

static int cs8409_dispatch_build_pcms(struct hda_codec *codec)
{
	struct cs8409_spec *spec = codec->spec;
	if (spec && spec->variant_ops && spec->variant_ops->build_pcms)
		return spec->variant_ops->build_pcms(codec);
	return 0;
}

static int cs8409_dispatch_init(struct hda_codec *codec)
{
	struct cs8409_spec *spec = codec->spec;
	if (spec && spec->variant_ops && spec->variant_ops->init)
		return spec->variant_ops->init(codec);
	return 0;
}

static void cs8409_dispatch_remove(struct hda_codec *codec)
{
	struct cs8409_spec *spec = codec->spec;
	if (spec && spec->variant_ops && spec->variant_ops->remove)
		spec->variant_ops->remove(codec);
}

static void cs8409_dispatch_unsol_event(struct hda_codec *codec, unsigned int res)
{
	struct cs8409_spec *spec = codec->spec;
	if (spec && spec->variant_ops && spec->variant_ops->unsol_event)
		spec->variant_ops->unsol_event(codec, res);
}

static int cs8409_dispatch_suspend(struct hda_codec *codec)
{
	struct cs8409_spec *spec = codec->spec;
	if (spec && spec->variant_ops && spec->variant_ops->suspend)
		return spec->variant_ops->suspend(codec);
	return 0;
}

static int cs8409_dispatch_resume(struct hda_codec *codec)
{
	struct cs8409_spec *spec = codec->spec;
	if (spec && spec->variant_ops && spec->variant_ops->resume)
		return spec->variant_ops->resume(codec);
	return 0;
}

static int cs8409_dispatch_check_power_status(struct hda_codec *codec, hda_nid_t nid)
{
	struct cs8409_spec *spec = codec->spec;
	if (spec && spec->variant_ops && spec->variant_ops->check_power_status)
		return spec->variant_ops->check_power_status(codec, nid);
	return 0;
}

static const struct hda_codec_ops cs8409_codec_ops = {
	.probe = patch_cs8409,
	.build_controls = cs8409_dispatch_build_controls,
	.build_pcms = cs8409_dispatch_build_pcms,
	.init = cs8409_dispatch_init,
	.remove = cs8409_dispatch_remove,
	.unsol_event = cs8409_dispatch_unsol_event,
	.suspend = cs8409_dispatch_suspend,
	.resume = cs8409_dispatch_resume,
	.check_power_status = cs8409_dispatch_check_power_status,
};
